...
public class MemberController {
	...
	public String login(String mid, String mpassword,HttpSession session,Model model){
		...
		if(result == MemberService.LOGIN_FAIL_PASSWORD){
	...
	public String join(Member member){
		try{
			...
			if(result == MemberService.JOIN_FAIL){return "member/joinForm";}
			return "redirect:/member/login";
		}catch (Exception e) {
			return "member/joinForm";
		}
	...
	public String logout(HttpSession session){
		int result = memberService.logout((String)session.getAttribute("login"));
